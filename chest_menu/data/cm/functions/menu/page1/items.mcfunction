# Notes
  ...
  # Head set actions are performed in items_<character> instead of directory_<character> to avoid miscalculations due to delayed updates

# Count Supplies
function cm:menu/page1/count_supplies

# Character Selection
  # Dolores
  execute if score alive DoloresStatus matches 1 unless score characterSent ChestMenuExplore matches 1 run function cm:menu/page1/items_dolores
  ...
  # Mary
  execute if score alive MaryStatus matches 1 unless score characterSent ChestMenuExplore matches 2..3 run function cm:menu/page1/items_mary
  ...
  # Ted
  execute if score alive TedStatus matches 1 unless score characterSent ChestMenuExplore matches 4 run function cm:menu/page1/items_ted
  ...
  # Timmy
  execute if score alive TimmyStatus matches 1 unless score characterSent ChestMenuExplore matches 5 run function cm:menu/page1/items_timmy
...
# Mass Items
  # Soup
    ...
    # Lock
    execute if score cans ItemsBunker < totalPresent GameStatus run data modify block 0 54 29 Items insert 0 value {Slot:21b,id:"minecraft:saddle",Count:1b,tag:{CustomModelData:102,isMenu:1b,page:1,selection:0,display:{Name:'{"text":"Locked","color":"red","bold":true}',Lore:['{"text":"Requires more soup","color":"dark_red"}']}}}

  # Water
    # Toggle
    execute if score bottles ItemsBunker >= totalPresent GameStatus if score bottlesGiven ChestMenuFeed < totalPresent GameStatus run data modify block 0 54 29 Items insert 0 value {Slot:23b,id:"minecraft:saddle",Count:1b,tag:{CustomModelData:79,isMenu:1b,page:1,selection:39,display:{Name:'{"text":"Keep all water","color":"gray","bold":true}',Lore:['{"text":"Click to add all","color":"dark_gray","underlined":true}']}}}
    ...
    # Lock
    execute if score bottles ItemsBunker < totalPresent GameStatus run data modify block 0 54 29 Items insert 0 value {Slot:23b,id:"minecraft:saddle",Count:1b,tag:{CustomModelData:105,isMenu:1b,page:1,selection:0,display:{Name:'{"text":"Locked","color":"red","bold":true}',Lore:['{"text":"Requires more water","color":"dark_red"}']}}}
...
# Page Buttons
  ...
  execute if score task AtomicDrill matches 14 if score bottlesGiven ChestMenuFeed >= totalPresent GameStatus run data modify block 0 54 29 Items insert 0 value {Slot:26b,id:"minecraft:saddle",Count:1b,tag:{CustomModelData:30,isMenu:1b,page:1,selection:1,display:{Name:'{"text":"Goto expedition","color":"gray","bold":true}',Lore:['{"text":"Click to continue","color":"dark_gray","underlined":true}']}}}
  # Submit
