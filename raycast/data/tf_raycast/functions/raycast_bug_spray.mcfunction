...
execute as @e[tag=!raycasting,type=!#tf_raycast:not_mob,dx=0] positioned ~-0.99 ~-0.99 ~-0.99 if entity @s[type=#tf_raycast:arthropods,dx=0] positioned ~0.99 ~0.99 ~0.99 run function tf_raycast:collide
...
execute if score .distance tf_rc matches ..70 positioned ^ ^ ^0.1 rotated ~ ~ if block ~ ~ ~ #tf_raycast:raycast_pass run function tf_raycast:raycast_bug_spray