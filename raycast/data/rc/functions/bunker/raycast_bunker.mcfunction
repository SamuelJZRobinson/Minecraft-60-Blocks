...
execute as @e[tag=!raycasting,type=!#rc:not_mob,dx=0] positioned ~-0.99 ~-0.99 ~-0.99 if entity @s[type=#rc:entities_bunker,tag=bunkerHitbox,dx=0] positioned ~0.99 ~0.99 ~0.99 run function rc:bunker/collide_bunker
...
execute if score .distance tf_rc matches ..90 positioned ^ ^ ^0.1 rotated ~ ~ if block ~ ~ ~ #rc:blocks_raycast_pass run function rc:bunker/raycast_bunker