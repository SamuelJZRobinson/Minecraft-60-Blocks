...
execute as @e[tag=!raycasting,type=!#rc:not_mob,dx=0] positioned ~-0.99 ~-0.99 ~-0.99 if entity @s[dx=0] positioned ~0.99 ~0.99 ~0.99 run function rc:expedition/collide
scoreboard players add .distance tf_rc 1

# Make raycast visible
function rc:utility/particles/expedition/gun_tracer
...
execute if score .distance tf_rc matches ..1280 positioned ^ ^ ^0.1 rotated ~ ~ if block ~ ~ ~ #rc:blocks_raycast_pass run function rc:expedition/raycast_gun