...
execute if score showEscape ItemsExpedition matches 0 if score itemsLeft ItemsExpedition matches 0 run tellraw @p[Mobsteam=Player] {"text":"Click to enter bunker","underlined":true,"color":"blue","clickEvent":{"action":"run_command","value":"/function tf_raycast:return_to_bunker"}}
