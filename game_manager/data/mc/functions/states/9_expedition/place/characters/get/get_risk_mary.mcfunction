# Increment Death Chance
  ...
  # Sick
  execute if score isSick MaryStatus matches 1 run scoreboard players operation deathChance ItemsExpedition += RISK_LEVEL_TWO ExpeditionOdds
  # Injuries
  ...
  # Crazy
  execute if score isCrazy MaryStatus matches 1 run scoreboard players operation deathChance ItemsExpedition += RISK_LEVEL_ONE ExpeditionOdds

# Copy Fatigue Status
scoreboard players operation isFatigued ItemsExpedition = isFatigued MaryStatus