...
execute if score expeditionDuration MenuExplore matches 1.. if score adultsAlive GameStatus matches 1 if score childrenAlive GameStatus matches 0 run schedule function mc:states/8_bunker/manage_day 1s

### Auto, Wait For Player Return Or Death. Seems redundant, already going to update bunker.
### execute if score doPovExpedition Settings matches 0 if score expeditionDuration MenuExplore matches 1.. if score characterSent MenuExplore matches 1.. run schedule function mc:states/8_bunker/update_bunker 1t replace
### execute if score doPovExpedition Settings matches 0 run schedule function mc:states/8_bunker/update_bunker 1t replace
