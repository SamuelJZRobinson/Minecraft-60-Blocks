...
# Death Chance
$execute if score isInjured $(objective) matches 1 if score injuredDays $(objective) >= INJURED_DAYS_CRITICAL CharactersLevels store result score output Math run function mc:utility/math/get_random_value_via_amount_score {"x":1,"score":INJURED_DEATH,"objective":CharactersOdds}
