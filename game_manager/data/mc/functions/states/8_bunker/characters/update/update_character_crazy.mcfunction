...
# Crazy
  ...
  # Crazy Chance
  $execute if score isCrazy $(objective) matches 0 if score sanity $(objective) = SANITY_AMOUNT_MIN CharactersLevels store result score output Math run function mc:utility/math/get_random_value_via_amount_score {"x":1,"score":CRAZY,"objective":CharactersOdds}
...
# Crazy Escape
  ...
  # Destroy Item Chance
  $execute if score isCrazyRage $(objective) matches 0 if score crazyDays $(objective) >= CRAZY_ESCAPE_DAYS_CRITICAL CharactersLevels store result score output Math run function mc:utility/math/get_random_value_via_amount_score {"x":1,"score":CRAZY_RAGE,"objective":CharactersOdds}
  ...
  $execute if score isCrazyRage $(objective) matches 0 if score crazyDays $(objective) >= CRAZY_ESCAPE_DAYS_CRITICAL CharactersLevels if score itemCount ItemsBunker matches 1.. if score output Math matches 1 run function mc:states/8_bunker/characters/update/manage_crazy_break_item
  ...
  # Escape Chance
  $execute if score crazyDays $(objective) >= CRAZY_ESCAPE_DAYS_CRITICAL CharactersLevels store result score output Math run function mc:utility/math/get_random_value_via_amount_score {"x":1,"score":CRAZY_ESCAPE,"objective":CharactersOdds}
