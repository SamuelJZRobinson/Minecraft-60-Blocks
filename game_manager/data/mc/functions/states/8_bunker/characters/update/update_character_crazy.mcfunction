...
# Crazy
  ...
  $execute if score isCrazy $(objective) matches 0 if score sanity $(objective) = SANITY_AMOUNT_MIN StatusLevels run function mc:utility/math/get_random_range with storage minecraft:math
...
# Crazy Escape
  ...
  $execute if score isCrazyRage $(objective) matches 0 if score crazyDays $(objective) >= CRAZY_ESCAPE_DAYS_CRITICAL StatusLevels run function mc:utility/math/get_random_range with storage minecraft:math
  ...
  $execute if score isCrazyRage $(objective) matches 0 if score crazyDays $(objective) >= CRAZY_ESCAPE_DAYS_CRITICAL StatusLevels if score itemCount ItemsBunker matches 1.. if score out Math matches 1 run function mc:states/8_bunker/characters/update/manage_break_item
  ...
  $execute if score crazyDays $(objective) >= CRAZY_ESCAPE_DAYS_CRITICAL StatusLevels run function mc:utility/math/get_random_range with storage minecraft:math
