...
execute if score crazy TedStatus matches 0 if score sanity TedStatus = SANITY_MIN StatusLevels run function mc:utility/math/get_random_range with storage minecraft:math
execute if score crazy TedStatus matches 0 if score sanity TedStatus = SANITY_MIN StatusLevels if score out Math matches 1 run scoreboard players set crazy TedStatus 1