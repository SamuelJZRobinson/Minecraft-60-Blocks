# Sick Chance
  # Low Soup
  $execute if score isSick $(objective) matches 0 if score soup $(objective) <= HUNGER_AMOUNT_CRITICAL CharactersLevels store result score output Math run function mc:utility/math/get_random_value_via_amount_score {"x":1,"score":SICK_STARVATION,"objective":CharactersOdds}
  ...
  # Low Water
  $execute if score isSick $(objective) matches 0 if score water $(objective) <= THIRST_AMOUNT_CRITICAL CharactersLevels store result score output Math run function mc:utility/math/get_random_value_via_amount_score {"x":1,"score":SICK_DEHYDRATION,"objective":CharactersOdds}
...
# Sick Death Chance
  ...
  # Chance
  $execute if score sickDays $(objective) >= SICK_DAYS_CRITICAL CharactersLevels store result score output Math run function mc:utility/math/get_random_value_via_amount_score {"x":1,"score":SICK_DEATH,"objective":CharactersOdds}
