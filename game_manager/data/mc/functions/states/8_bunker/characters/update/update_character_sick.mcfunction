# Sick Chance
  ...
  $execute if score isSick $(objective) matches 0 if score soup $(objective) <= HUNGER_AMOUNT_CRITICAL StatusLevels run function mc:utility/math/get_random_value with storage minecraft:math
  ...
  $execute if score isSick $(objective) matches 0 if score water $(objective) <= THIRST_AMOUNT_CRITICAL StatusLevels run function mc:utility/math/get_random_value with storage minecraft:math
...
# Sick Death Chance
  ...
  $execute if score sickDays $(objective) >= SICK_DAYS_CRITICAL StatusLevels run function mc:utility/math/get_random_value with storage minecraft:math
  $execute if score sickDays $(objective) >= SICK_DAYS_CRITICAL StatusLevels if score out Math matches 1 run scoreboard players set alive $(objective) -1
  # Guaranteed Death
