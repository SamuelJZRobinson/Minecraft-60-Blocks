...
# Player Related
  ...
  # Fake Blindness
  execute if score fakeBlindness GameStatus matches 1.. as @p[Mobsteam=Player] run function mc:utility/vision/manage_fake_blindness
  # Inventory
  execute if score lockInventory GameStatus matches 1..2 as @p[Mobsteam=Player] run function mc:utility/inventory/lock_inventory
  execute as @p[Mobsteam=Player,nbt=!{Inventory:[{id:"minecraft:written_book",Slot:8b,Count:1b,tag:{gameMenu:1b}}]}] run function mc:utility/inventory/give_game_menu
  # Clear Ground Items
