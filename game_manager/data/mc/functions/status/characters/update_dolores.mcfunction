...
# Fatigue
  ...
  execute if score fatigued DoloresStatus matches 0 if score day GameStatus >= FATIGUE_MIN StatusLevels if score day GameStatus <= FATIGUE_MAX StatusLevels run function mc:states/stages/5_setup_house/place/items/get_percentage
...
# Injuries
  ...
  execute if score alive DoloresStatus matches 1 if score sick DoloresStatus matches 0 if score infection DoloresStatus <= INFECTION_MIN StatusLevels run function mc:states/stages/5_setup_house/place/items/get_percentage
...
# Crazy
  ...
  execute if score alive DoloresStatus matches 1 if score crazy DoloresStatus matches 0 if score sanity DoloresStatus = SANITY_MIN StatusLevels run function mc:states/stages/5_setup_house/place/items/get_percentage
...
# Mental Breakdown
  ...
  execute if score alive DoloresStatus matches 1 if score crazyRage DoloresStatus matches 0 if score breakdown DoloresStatus <= BREAKDOWN_RAGE_MAX StatusLevels if score breakdown DoloresStatus > BREAKDOWN_MIN StatusLevels run function mc:states/stages/5_setup_house/place/items/get_percentage
  ...
  execute if score crazyRage DoloresStatus matches 1 if score itemCount ItemsBunker matches 1.. run function mc:status/characters/load_crazy_dolores
  scoreboard players set math_out em 0
  # Breakdown Chance (1 in x, n+1)
  scoreboard players operation memory em = DOLORES_RESISTANCE StatusOdds
  execute if score alive DoloresStatus matches 1 if score breakdown DoloresStatus <= BREAKDOWN_MIN StatusLevels run function mc:states/stages/5_setup_house/place/items/get_percentage
...
# Sickness
  ...
  # Malnutrition Chance (1 in x, n+1)
    ...
    execute if score alive DoloresStatus matches 1 if score sick DoloresStatus matches 0 if score soup DoloresStatus > HUNGER_MIN StatusLevels if score water DoloresStatus <= THIRST_MIN StatusLevels run function mc:states/stages/5_setup_house/place/items/get_percentage
    # No Soup, Has Water
  ...
  execute if score alive DoloresStatus matches 1 if score sickness DoloresStatus <= SICKNESS_MIN StatusLevels run function mc:states/stages/5_setup_house/place/items/get_percentage
...
# Decorate Bunker
  # Spawn
  execute if score alive DoloresStatus matches 1 unless entity @e[x=-2,y=54,z=31,dy=4,dz=1,type=minecraft:armor_stand,tag=bunkerDolores] run function mc:status/characters/spawn_dolores
  ...
  # Update Death
  execute if score alive DoloresStatus matches 0 as @e[x=-2,y=54,z=31,dy=4,dz=1,type=minecraft:armor_stand,tag=bunkerDolores] run function mc:status/characters/update_dolores_dead