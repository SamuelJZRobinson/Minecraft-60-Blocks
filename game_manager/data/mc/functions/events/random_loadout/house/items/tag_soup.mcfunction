...
# Tag Items For Each Room
  ...
  execute if score room ItemsHouse matches 1 run teleport @s @e[type=marker,limit=1,sort=random,tag=kitchen,tag=ground]
  ...
  execute if score room ItemsHouse matches 2 run teleport @s @e[type=marker,limit=1,sort=random,tag=hallway,tag=ground]
...
# Call Root
execute if score soupLeft ItemsHouse matches 0 run function mc:events/random_loadout/house/items/refresh_scores
