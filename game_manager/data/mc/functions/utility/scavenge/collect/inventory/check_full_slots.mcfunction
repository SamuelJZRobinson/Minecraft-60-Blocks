...
# Remove Effects
# execute as @p[team=Player,nbt={ActiveEffects:[{Id:18b}]}] if score fullSlots GameStatus matches ..3 run effect clear @s weakness
...
# Title
execute unless score task AtomicDrill matches 1..6 if score fullSlots GameStatus matches 4 run title @p[team=Player] actionbar {"text":"Inventory full!","color":"red","bold":true}