...
# Fatigue
  ...
  execute if score fatigued TedStatus matches 0 if score day GameStatus >= FATIGUE_MIN StatusLevels if score day GameStatus <= FATIGUE_MAX StatusLevels run function mc:events/random_loadout/house/items/get_percentage
...
# Injuries
  ...
  execute if score alive TedStatus matches 1 if score sick TedStatus matches 0 if score infection TedStatus <= INFECTION_MIN StatusLevels run function mc:events/random_loadout/house/items/get_percentage
...
# Crazy
  ...
  execute if score alive TedStatus matches 1 if score crazy TedStatus matches 0 if score sanity TedStatus = SANITY_MIN StatusLevels run function mc:events/random_loadout/house/items/get_percentage
...
# Mental Breakdown
  ...
  execute if score breakdown TedStatus matches 0 run scoreboard players set escaped TedStatus 1
  ...
  execute if score alive TedStatus matches 1 if score crazyRage TedStatus matches 0 if score breakdown TedStatus <= BREAKDOWN_RAGE_MAX StatusLevels if score breakdown TedStatus > BREAKDOWN_MIN StatusLevels run function mc:events/random_loadout/house/items/get_percentage
  ...
  execute if score alive TedStatus matches 1 if score breakdown TedStatus <= BREAKDOWN_MIN StatusLevels run function mc:events/random_loadout/house/items/get_percentage
...
# Sickness
  ...
  # Malnutrition Chance (1 in x, n+1)
    ...
    execute if score alive TedStatus matches 1 if score sick TedStatus matches 0 if score soup TedStatus > HUNGER_MIN StatusLevels if score water TedStatus <= THIRST_MIN StatusLevels run function mc:events/random_loadout/house/items/get_percentage
    # No Soup, Has Water
  ...
  execute if score alive TedStatus matches 1 if score sickness TedStatus <= SICKNESS_MIN StatusLevels run function mc:events/random_loadout/house/items/get_percentage
...
# Decorate Bunker
  # Spawn
  execute if score alive TedStatus matches 1 unless entity @e[x=0,y=54,z=31,dy=4,dz=1,type=minecraft:armor_stand,tag=bunkerTed] run function mc:status/characters/spawn_ted
  ...
  # Update Death
  execute if score alive TedStatus matches 0 as @e[x=0,y=54,z=31,dy=4,dz=1,type=minecraft:armor_stand,tag=bunkerTed] run function mc:status/characters/update_ted_dead